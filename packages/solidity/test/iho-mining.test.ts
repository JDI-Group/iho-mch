...
import { randomNumer } from '@hairy/utils'
import { isAddress, solidityPackedKeccak256, toBeArray } from 'ethers'
...
async function sign(data: BigNumberish) {
  ...
  return verifier.signMessage(toBeArray(data))
}

function generateDeviceId() {
  return `device-${Math.floor(randomNumer(1000, 9999))}`
}

function generateRewardId() {
  return `reward-${Math.floor(randomNumer(1000, 9999))}`
}
